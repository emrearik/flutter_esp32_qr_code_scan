...
void loop()
{
  ...
  if (reader.receiveQrCode(&qrCodeData, 1000))
  {
    ...
    //veriler geçerli mi
    {
      ...
      //path belirlenir.
      String documentPath = "codes/user_" + String(count);

      //parçalanan qr kod datası arraylara atanır.
      content.set("fields/name/stringValue/", array[0]);
      content.set("fields/surname/doubleValue/", array[1]);

      //kullanıcı gönderilir
      if (Firebase.Firestore.createDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), content.raw()))
        Serial.printf("Gonderiliyor...\n%s\n\n", fbdo.payload().c_str());
      else
